// ============================================
// XPLITLEAKS API - CLOUDFLARE WORKER (COMPLETE)
// Fixes: Report Auth, R2 URL Safety, Upload Logic
// ============================================
export default {
...
const method = request.method;
    ...
    try {
        ...
        // Report Content (FIXED: Moved to public - was incorrectly behind admin auth)
        if (path === "/api/report" && method === "POST") {
            const { contentId, contentType, reason, details, reporterEmail, reporterName } = await request.json().catch(() => ({}));
            
            if (!contentId || !contentType || !reason) {
                return errorResponse("Missing required fields (contentId, contentType, reason)", 400);
            }
            
            await env.DB.prepare(`
                INSERT INTO reports (contentId, contentType, reason, details, reporterEmail, reporterName, reporterSession, createdAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
            `).bind(
                contentId, 
                contentType, 
        ...
        // List Videos with Pagination, Search, and Category Filtering
        if (path === "/api/videos" && method === "GET") {
            ...
            // Get videos with calculated display views (fake + real)
        ...
        // Track Video View (Detailed Analytics)
...
// ============================================
// SECTION 4: CREATOR AUTHENTICATION & UPLOADS
// ============================================
...
// ============================================
// SECTION 5: ADMIN ENDPOINTS & FALLBACK
// ============================================

// Admin Auth Gate (All routes below require admin token)
if (!checkAdminAuth()) {
    return errorResponse("Unauthorized. Admin Token Required.", 401);
}
...
if (path === "/api/admin/stats" && method === "GET") {
    ...
    // Daily upload stats for the last 30 days
...
// Get All Creators with Content Counts
...
// Update Creator Status (Approve/Reject/Suspend)
